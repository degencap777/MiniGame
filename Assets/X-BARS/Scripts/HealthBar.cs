...
public class HealthBar : MonoBehaviour {
	...
	void Start () {
        ...
        if(!healthbarRoot.GetComponent<HealthbarRoot>().healthBars.Contains(HealthbarPrefab))
            healthbarRoot.GetComponent<HealthbarRoot>().healthBars.Add(HealthbarPrefab);
	...
	void FixedUpdate () {
		if(!HealthbarPrefab)
			return;
        //Vector2 pos;
        //Vector3 tempV3 = cam.WorldToScreenPoint(transform.position);
        //RectTransformUtility.ScreenPointToLocalPointInRectangle(camCanvas.transform as RectTransform, new Vector2(tempV3.x, tempV3.y), camCanvas.worldCamera, out pos);
        //HealthbarPrefab.transform.localPosition = Vector3.Lerp(HealthbarPrefab.transform.localPosition, new Vector3(pos.x, pos.y+yOffset, 0),Time.fixedDeltaTime*50);
	    HealthbarPrefab.transform.position = cam.WorldToScreenPoint(new Vector3(thisT.position.x, thisT.position.y , thisT.position.z))+new Vector3(0,yOffset,0);
    ...
    bool OutDistance()
    {
        //if (Mathf.Abs(HealthbarPrefab.transform.position.y) > 540 ||
        //    Mathf.Abs(HealthbarPrefab.transform.position.x) > 960)
        //    return true;
        //return false;
