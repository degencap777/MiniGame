...
public class RTS_NewSyntaxe : MonoBehaviour {
	...
	void Update () {
		...
		// Cube
		if (current != null && current.type == EasyTouch.EvtType.On_SimpleTap && current.pickedObject !=null && current.pickedObject.name=="Cube"){
			ResteColor();
		...
		// Pinch
		if (current != null && current.type == EasyTouch.EvtType.On_Pinch ){
			Camera.main.fieldOfView += current.deltaPinch * 10 * Time.deltaTime;
		}

		// Twist
		if (current != null && current.type == EasyTouch.EvtType.On_Twist ){
